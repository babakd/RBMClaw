...
fun SettingsSheet(viewModel: MainViewModel) {
  ...
  val rbmEnabled by viewModel.rbmEnabled.collectAsState()
  val rbmUseForTalkVision by viewModel.rbmUseForTalkVision.collectAsState()
  val rbmHighResolution by viewModel.rbmHighResolution.collectAsState()
  val rbmRegistrationState by viewModel.rbmRegistrationState.collectAsState()
  val rbmDevices by viewModel.rbmDevices.collectAsState()
  val rbmStreamState by viewModel.rbmStreamState.collectAsState()
  ...
  LazyColumn(
    ...
    // Order parity: Node → Gateway → Voice → RBM → Camera → Messaging → Location → Screen.
    ...
    // Ray-Ban Meta
    item { Text("Ray-Ban Meta", style = MaterialTheme.typography.titleSmall) }
    item {
      val rbmStatus =
        "Reg ${rbmRegistrationLabel(rbmRegistrationState)} \u00b7 Devices ${rbmDevices.size} \u00b7 Stream ${rbmStreamLabel(rbmStreamState)}"
      ListItem(
        headlineContent = { Text("Enable RBM Vision Stream") },
        supportingContent = { Text(rbmStatus) },
        trailingContent = {
          Switch(
            checked = rbmEnabled,
            onCheckedChange = viewModel::setRbmEnabled,
          )
        },
      )
    }
    item {
      Button(onClick = viewModel::startRbmRegistration) {
        Text("Register with Meta AI")
      }
    }
    item {
      Button(onClick = viewModel::startRbmUnregistration) {
        Text("Unregister from Meta AI")
      }
    }
    item {
      ListItem(
        headlineContent = { Text("Use RBM for Talk Vision") },
        supportingContent = { Text("Attach the latest glasses frame to each Talk Mode request.") },
        trailingContent = {
          Switch(
            checked = rbmUseForTalkVision,
            onCheckedChange = viewModel::setRbmUseForTalkVision,
            enabled = rbmEnabled,
          )
        },
      )
    ...
    item {
      ListItem(
        headlineContent = { Text("High Resolution Stream") },
        supportingContent = { Text("Higher frame quality with higher battery/network cost.") },
        trailingContent = {
          Switch(
            checked = rbmHighResolution,
            onCheckedChange = viewModel::setRbmHighResolution,
            enabled = rbmEnabled,
          )
        },
      )
    ...
    item {
      Text(
        "RBM streaming runs only when connected to a gateway.",
        color = MaterialTheme.colorScheme.onSurfaceVariant,
      )
    }
...
private fun rbmRegistrationLabel(state: RegistrationState): String {
  val raw = state::class.simpleName?.trim().orEmpty().ifEmpty { state.toString() }
  return raw
    .replace(Regex("([a-z])([A-Z])"), "$1 $2")
    .lowercase()
}

private fun rbmStreamLabel(state: StreamSessionState): String {
  return state.toString().replace('_', ' ').lowercase()
}
